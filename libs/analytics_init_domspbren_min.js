...
$LAB.script("//www.googletagmanager.com/gtm.js?id=" + config.tagmanager_id + config.dataLayer_url).script(config.plugins_path).script("//www.google-analytics.com/analytics" + (config.debug == true ? "_debug" : "") + ".js").wait(function() {
	...
	config.calltr = $LAB.sandbox();
	config.calltr.script("//mod.calltouch.ru/d_client.js?param;client_id" + config.uid.userId + ";ref" + encodeURI(config.ref) + ";url" + encodeURI(config.loc.href.split("#")[0]) + ";cook" + encodeURI(config.ck));
